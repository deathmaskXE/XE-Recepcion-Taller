...
await setDoc(doc(db,"estados_publicos",folio),pub);$("created").textContent=`FOLIO CREADO: ${folio}`;["cliente","telefono","equipo","modelo","falla","nota"].forEach(x=>$(x).value="")};
function listen(){onSnapshot(query(collection(db,"equipos"),orderBy("recibido","desc")),s=>{all=s.docs.map(x=>({id:x.id,...x.data()}));render()})}$("filter").oninput=render;
function render(){let f=$("filter").value.toLowerCase(),arr=all.filter(x=>(x.id+" "+x.cliente+" "+x.equipo).toLowerCase().includes(f));$("list").innerHTML=arr.map(x=>`<div class="item"><div class="itemtop"><div><h3>${x.id} · ${esc(x.equipo)}</h3><p>${esc(x.cliente)} · ${esc(x.falla||"Sin falla reportada")}</p></div><b>${x.estado}</b></div><div class="controls"><select data-state="${x.id}">${states.map(s=>`<option ${s===x.estado?"selected":""}>${s}</option>`).join("")}</select><textarea data-note="${x.id}">${esc(x.nota||"")}</textarea><button data-save="${x.id}">GUARDAR</button></div></div>`).join("");document.querySelectorAll("[data-save]").forEach(b=>b.onclick=async()=>{let id=b.dataset.save,estado=document.querySelector(`[data-state="${id}"]`).value,nota=document.querySelector(`[data-note="${id}"]`).value,upd={estado,nota,actualizado:Date.now()};if(estado==="Entregado"){let old=all.find(x=>x.id===id);upd.entregado=old.entregado||Date.now()}await updateDoc(doc(db,"equipos",id),upd);await updateDoc(doc(db,"estados_publicos",id),upd)})}
